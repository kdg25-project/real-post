...
export default function AdminBottomNav() {
    ...
    return(
        <nav className='fixed items-center bottom-0 left-0 flex w-full h-[70px] bg-white px-[10px] shadow-base z-[10]'>
            <Link href="/admin/edit" className={`w-full text-center ${pathname === "/admin/edit" ? "text-primary" : "text-gray-dark"}`}>
                <SquarePen size={28} className='mx-auto'></SquarePen>
                <p className='text-[12px] font-medium'>Edit</p>
            </Link>
            <Link href="/admin/qr" className={`w-full text-center ${pathname === "/admin/qr" ? "text-primary" : "text-gray-dark"}`}>
